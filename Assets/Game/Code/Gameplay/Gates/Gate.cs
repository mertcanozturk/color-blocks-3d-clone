...
namespace ColorBlocks
{
    public class Gate : MonoBehaviour
    {
        ...
        public void Init(int row, int col, int direction, int color)
        {
            ...
            if(!_isInitialized)
            {
                _smashblockStartPosition = smashBlock.transform.localPosition;
                _shakeObjectStartPosition = shakeObject.localPosition;
                _gateStartPosition = gateTransform.localPosition;
                _isInitialized = true;
            }

            gateTransform.localPosition = _gateStartPosition;
            smashBlock.transform.localPosition = _smashblockStartPosition;
        ...
        private IEnumerator SmashBlockCoroutine(Block block)
        {
            ...
            smashBlock.transform.localPosition = _smashblockStartPosition;
            shakeObject.localPosition = _shakeObjectStartPosition;
            sequence.Append(smashBlock.transform.DOLocalMove(targetPosition, 1f).SetEase(Ease.Linear));
            ...
            gateSequence.Append(gateTransform.DOLocalMove(_gateStartPosition - Vector3.up * 1.5f, 0.1f).SetEase(Ease.InQuad));
            gateSequence.Append(gateTransform.DOLocalMove(_gateStartPosition, 0.75f));
